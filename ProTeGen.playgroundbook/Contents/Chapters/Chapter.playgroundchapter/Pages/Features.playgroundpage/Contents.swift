...
/*:
- Experiment:
*To change how much of the world is water, you can simply change the value used in the rule. Here, one means only where the ground is the lowest. Zero would make water never occur, as the* `waterTable` *would always be below ground. Notice how if you make it higher, no surface features will be drawn in the scene.* **Tap "Run My Code" to see the world that is made!**
 */
let waterLevel = /*#-editable-code*/1/*#-end-editable-code*/

let world = World(worldWidth, worldHeight)
world.generate()
//: Next, we'll move onto adding some differing [details](Details).
//#-hidden-code
scene.draw(world)
playSound("forest")
//#-end-hidden-code
